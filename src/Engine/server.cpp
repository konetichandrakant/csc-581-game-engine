...
static bool bind_tcp(void* s, const char* host, int port){
    char ep[128]; std::snprintf(ep,sizeof(ep),"tcp://%s:%d",host,port); return zmq_bind(s, ep)==0;
}


void cmdLoop(std::atomic<bool>* running, const char* host, int cmdPort, void* ctx) {
    void* rep = zmq_socket(ctx, ZMQ_REP); if (!rep) { std::cerr << "[Server] REP socket failed\n"; return; }
    set_linger0(rep);
    if (!bind_tcp(rep, host, cmdPort)) { std::cerr << "[Server] bind REP failed\n"; zmq_close(rep); return; }
    int nextId = 1;
    while (running->load()) {
        uint8_t buf[512];
        int n = zmq_recv(rep, buf, sizeof(buf), 0);
        if (n <= 0) { std::this_thread::sleep_for(std::chrono::milliseconds(1)); continue; }
        MsgKind kind = *(MsgKind*)buf;
...
void dirLoop(std::atomic<bool>* running, const char* host, int dirPort, void* ctx) {
    void* rep = zmq_socket(ctx, ZMQ_REP); if (!rep) { std::cerr << "[Server] DIR REP socket failed\n"; return; }
    set_linger0(rep);
    if (!bind_tcp(rep, host, dirPort)) { std::cerr << "[Server] bind DIR REP failed\n"; zmq_close(rep); return; }
    ...
    while (running->load()) {
        uint8_t buf[1024];
        int n = zmq_recv(rep, buf, sizeof(buf), 0);
        if (n < (int)sizeof(P2DRegister)) { zmq_send(rep,"",0,0); continue; }
        auto* reg = (P2DRegister*)buf;
        ...
        PeerInfo pi{};
        pi.ipv4_be = htonl(0);
        pi.port_be = htons(reg->pub_port);
        pi.lastSeen = (uint64_t)std::chrono::steady_clock::now().time_since_epoch().count();
        peers[id] = pi;

        std::vector<P2DPeerEndpoint> list;
        list.reserve(peers.size());
        for (auto& [pid, info] : peers) if (pid != id) list.push_back({pid, info.ipv4_be, info.port_be});

        std::vector<uint8_t> out;
        out.resize(sizeof(P2DReply) + list.size()*sizeof(P2DPeerEndpoint));
        auto* r = (P2DReply*)out.data();
        r->h.kind = P2PKind::DirReply; r->h.tick = 0;
        r->my_player_id = id; r->peer_count = (uint32_t)list.size();
        if (!list.empty()) std::memcpy(out.data()+sizeof(P2DReply), list.data(), list.size()*sizeof(P2DPeerEndpoint));
        zmq_send(rep, out.data(), (int)out.size(), 0);
    }
    zmq_close(rep);
}
